...
function DataGridColumnHeaderInner<TData, TValue>({
  ...
  const headerLabelClassName = cn(
    "text-secondary-foreground/80 inline-flex h-full items-center gap-1 font-normal [&_svg]:opacity-60 text-[10px] leading-tight [&_svg]:size-2.5",
    className
  )

  const headerButtonClassName = cn(
    "text-secondary-foreground/80 hover:bg-secondary data-[state=open]:bg-secondary hover:text-foreground data-[state=open]:text-foreground -ms-1 h-5 min-h-5 gap-1 px-1.5 text-[10px] leading-none font-medium rounded-md [&_svg]:size-2.5 [&_svg]:shrink-0",
    className
  )
  ...
  const sortIcon =
    ...
    (isSorted === "desc" ? (
      ...
      <ChevronsUpDownIcon className="size-2.5 opacity-70" />
    ))
